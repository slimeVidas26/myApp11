...
export const  OrderDetailScreenAll= ()=> {


  const context = useContext(EDIContext)
  const {data , ref , searchIcon , initialRows , allTab} = context


 
  
const remindRows = allTab.length;
...
const getItemOrder = async()=>{    
  ...
  resolve(allTab.filter(item => item.code  === number))});
